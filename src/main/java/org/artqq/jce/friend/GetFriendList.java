...
public class GetFriendList {
    ...
    public synchronized static byte[] getFriendGroupList(final int r, ArtBot bot,
                                            Integer friendStartIndex,
                                            Integer friendCount,
                                            Integer groupStartIndex,
                                            Integer groupCount,
                                            Integer ifShowTermType){
        return getFriendGroupList(r, bot, friendStartIndex.shortValue(), friendCount.shortValue(), groupStartIndex.byteValue(), groupCount.byteValue(), ifShowTermType.byteValue());
    }
    
    public synchronized static byte[] getFriendGroupList(int id, ArtBot bot,
                                                 short friendStartIndex,
                                                 short friendCount,
                                                 byte groupStartIndex,
                                                 byte groupCount,
                                                 byte ifShowTermType){
        ArrayList<Long> snsTypelist = new ArrayList<Long>();
        snsTypelist.add(13580L);
        snsTypelist.add(13581L);
        snsTypelist.add(13582L);
        Req req = new Req();
        req.reqtype = 3;
        req.ifReflush = friendStartIndex > 0 ? (byte) 0 : 1;
        req.uin = bot.getUin();
        req.startIndex = friendStartIndex;
        req.getfriendCount = friendCount;
        req.groupid = 0;
        req.ifGetGroupInfo = friendStartIndex > 0 ? (byte) 0 : 1;
        req.getgroupCount = groupCount;
        req.ifGetMSFGroup = 0;
        req.ifShowTermType = ifShowTermType;
        req.version = 31;
        req.eAppType = 0;
        req.ifGetDOVId = 0;
        req.ifGetBothFlag = 0;
        ...
        req.vecSnsTypelist = snsTypelist;
        byte[] arr = QQAgreementUtils.encodePacket(req, "mqq.IMService.FriendListServiceServantObj", "GetFriendListReq", "FL", id);
        return arr;
    }
}
